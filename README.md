...
│   ├── llm.py           # OpenAI integration & streaming logic
...
├── setup_db.sql         # Supabase database schema
└── README.md            # This file
```
...
### 3. Chat with AI
Try these examples:
- `"Hello, I'm [Your Name]"` — Tests greeting & context
- `"What is 10 + 2?"` — Tests simple Q&A
- `"How much to ship 5kg to the US?"` — Tests tool calling
- `"What is my name?"` — Tests conversation memory

### 4. Verify in Supabase
- **Table Editor → sessions**: See active/closed sessions
- **Table Editor → session_events**: See all messages & tool calls

### 5. Test Summarization
- Click **Disconnect**
- Check `sessions` table — `summary` and `end_time` columns will be populated
...
- Frontend remains "dumb" — only sends/receives text
